...
void LWIP_Init(void)
{
	...
	Netif_LinkSemaphore = xSemaphoreCreateBinary();

	// Create the Ethernet link handler thread
	link_arg.netif = &gnetif;
	link_arg.semaphore = Netif_LinkSemaphore;
	xTaskCreate((TaskFunction_t)ethernetif_set_link, "ETH_LINK_TASK", configMINIMAL_STACK_SIZE * 2, &link_arg, 2, NULL);
}
...
{
	xSemaphoreGiveFromISR(s_xSemaphore, 0);
}
...
static void low_level_init(struct netif *netif)
{
	...
	xTaskCreate((TaskFunction_t)ethernetif_input, "ETH_RX_TASK", INTERFACE_THREAD_STACK_SIZE, netif, 6, NULL);
...
void ethernetif_input(void const * argument)
{
	...
	while(1) {
		if (xSemaphoreTake(s_xSemaphore, TIME_WAITING_FOR_INPUT)) {
			do {
				LOCK_TCPIP_CORE();
...
err_t ethernetif_init(struct netif *netif)
{
	...
	// Initialize interface host name
	netif->hostname = "lwip";
#endif
